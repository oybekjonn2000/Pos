...
import { KitchenService, KitchenStation, KitchenOrderBatch } from '../../core/services/kitchen.service';
...
@Component({
              ...
              <div class="cart-item" [class.cart-item--new]="item.isNew" [class.cart-item--voided]="item.voided">
                <div class="cart-item__main">
                  ...
                  <div class="cart-item__content">
                    ...
                    <!-- 2. Meta Row: Unit Price + Status Pill + Sent Breakdown -->
                    <div class="cart-item__meta-row">
                      ...
                      @if ((item.sentQuantity || 0) > 0) {
                        <span class="sent-qty-badge">
                          Yuborilgan: {{ item.sentQuantity }}x
                        </span>
                      }
                      @if (!item.voided && item.quantity > (item.sentQuantity || 0)) {
                        <span class="unsent-qty-badge">
                          Yangi: +{{ item.quantity - (item.sentQuantity || 0) }}x
                        </span>
                      }
                      @if (item.voidReason) {
        ...
        <div class="modal-backdrop" (click)="showRoundsModal.set(false)">
          <div class="modal-card rounds-modal-card" (click)="$event.stopPropagation()">
            ...
            <div class="modal-body">
              @if (loadingBatches()) {
                <div class="spinner"></div>
                <p style="text-align: center; color: var(--text-secondary);">Partiyalar yuklanmoqda...</p>
              } @else if (orderBatches().length === 0) {
                <p style="text-align: center; color: var(--text-secondary); padding: 24px;">Ushbu buyurtma bo'yicha oshxonaga partiyalar hali yuborilmagan.</p>
              } @else {
                <div class="rounds-list">
                  @for (b of orderBatches(); track b.id) {
                    <div class="round-card" [class.round-card--addon]="b.batchType === 'ADDON' || b.batchNumber > 1">
                      ...
                      <div class="round-items">
                        @for (it of b.items; track it.id) {
                          <div class="round-item-row">
                            <span class="round-qty">{{ it.quantity }}x</span>
                            <span class="round-name">{{ it.productName }}</span>
                            <span class="status-pill" [class]="'pill--' + (it.status || 'NEW').toLowerCase()">{{ getBatchStatusLabel(it.status) }}</span>
                          </div>
                        }
                      </div>
                    </div>
            ...
            <div class="modal-footer" style="margin-top: 16px; display: flex; justify-content: flex-end;">
              <button class="btn btn--secondary" (click)="showRoundsModal.set(false)">Yopish</button>
            </div>
          </div>
        </div>
      }
  ...
  styles: [`
    ...
    .status-pill {
      ...
      &.pill--delivered, &.pill--served {
        background: rgba(100, 116, 139, 0.15);
        color: #94a3b8;
        border: 1px solid rgba(100, 116, 139, 0.3);
      }
      &.pill--cancelled, &.pill--voided {
...
export class PosComponent implements OnInit {
  ...
  // Rounds / Batches modal state
  showRoundsModal = signal(false);
  orderBatches = signal<KitchenOrderBatch[]>([]);
  ...
  openRoundsModal(): void {
    const orderId = this.currentOrderId();
    if (!orderId) return;
    this.showRoundsModal.set(true);
    this.loadingBatches.set(true);
    this.orderService.getOrderBatches(orderId).subscribe({
      next: (res) => {
        this.loadingBatches.set(false);
        this.orderBatches.set(res.data || []);
      },
      error: (err) => {
        this.loadingBatches.set(false);
        console.error('Failed to load order batches', err);
      }
    });
  }
  ...
  formatBatchTime(isoDate?: string): string {
    if (!isoDate) return '';
    try {
