...
import com.restaurantpos.kitchen.dto.KitchenBatchDto;
...
public class OrderService {
    ...
    public OrderDto.Response sendNewItemsToKitchen(UUID orderId, UUID tenantId, com.restaurantpos.auth.security.UserPrincipal user, OrderDto.SendToKitchenRequest request) {
        ...
        if (user != null && user.getUserId() != null) {
            sender = userRepository.findById(user.getUserId()).orElse(null);
        }

        List<KitchenOrderBatch> createdBatches = createAndRouteKitchenBatches(order, sender);

        if (createdBatches.isEmpty()) {
            boolean hasAlreadySentItems = order.getItems() != null && order.getItems().stream()
                    .anyMatch(i -> i.getKitchenStatus() != OrderItem.KitchenStatus.NEW && i.getSentQuantity() != null && i.getSentQuantity().compareTo(BigDecimal.ZERO) > 0);
            if (hasAlreadySentItems) {
                log.info("Order {} send-to-kitchen duplicate request ignored gracefully (all items already dispatched).", order.getOrderNumber());
                return toResponse(order);
            }
    ...
    private void routeOrderToKitchens(Order order, Tenant tenant) {
        ...
        createAndRouteKitchenBatches(order, order.getWaiter());
        wsNotification.notifyNewOrder(tenant.getId(), toResponse(order));
    }

    private List<KitchenOrderBatch> createAndRouteKitchenBatches(Order order, User senderUser) {
        if (order.getItems() == null || order.getItems().isEmpty()) {
            return Collections.emptyList();
        }

        // Filter strictly ONLY items with remainingQuantity > 0, not voided, kitchen != null
        ...
        int nextBatchNumber = kitchenOrderBatchRepository.findTopByOrderIdOrderByBatchNumberDesc(order.getId())
                .map(b -> b.getBatchNumber() + 1)
                .orElse(1);

        KitchenOrderBatch.BatchType batchType = (nextBatchNumber == 1)
                ? KitchenOrderBatch.BatchType.INITIAL
                : KitchenOrderBatch.BatchType.ADDON;

        // Group by kitchen
        Map<com.restaurantpos.kitchen.entity.Kitchen, List<OrderItem>> itemsByKitchen = itemsToSend.stream()
                .collect(Collectors.groupingBy(OrderItem::getKitchen));

        Instant now = Instant.now();
        List<KitchenOrderBatch> createdBatches = new ArrayList<>();

        for (Map.Entry<com.restaurantpos.kitchen.entity.Kitchen, List<OrderItem>> entry : itemsByKitchen.entrySet()) {
            com.restaurantpos.kitchen.entity.Kitchen kitchen = entry.getKey();
            List<OrderItem> kitchenItems = entry.getValue();

            KitchenOrderBatch batch = new KitchenOrderBatch();
            batch.setTenant(order.getTenant());
            batch.setOrder(order);
            batch.setKitchen(kitchen);
            batch.setBatchNumber(nextBatchNumber);
            batch.setBatchType(batchType);
            batch.setStatus(KitchenOrderBatch.BatchStatus.NEW);
            batch.setNotes(order.getKitchenNotes() != null ? order.getKitchenNotes() : order.getNotes());
            batch.setCreatedBy(senderUser != null ? senderUser : order.getWaiter());
            batch.setCreatedAt(now);
            batch.setSentAt(now);

            KitchenOrderBatch savedBatch = kitchenOrderBatchRepository.save(batch);

            for (OrderItem item : kitchenItems) {
                BigDecimal unsentQty = item.getRemainingQuantity();

                KitchenOrderBatchItem batchItem = new KitchenOrderBatchItem();
                batchItem.setTenant(order.getTenant());
                batchItem.setBatch(savedBatch);
                batchItem.setOrderItem(item);
                batchItem.setProduct(item.getProduct());
                batchItem.setProductName(item.getProductName());
                batchItem.setProductSku(item.getProductSku());
                batchItem.setQuantity(unsentQty);
                batchItem.setUnitPrice(item.getUnitPrice());
                batchItem.setStatus(KitchenOrderBatchItem.ItemStatus.NEW);
                batchItem.setNotes(item.getNotes());
                batchItem.setCreatedAt(now);
                batchItem.setUpdatedAt(now);

                savedBatch.getItems().add(kitchenOrderBatchItemRepository.save(batchItem));

                // Update OrderItem sentQuantity
                item.setSentQuantity(item.getSentQuantity().add(unsentQty));
            ...
            // Backward compatibility KitchenTicket record
            ...
            ticket.setTicketNumber(order.getOrderNumber() + "-" + kitchen.getCode() + "-B" + nextBatchNumber);
            ticket.setStatus(com.restaurantpos.kitchen.entity.KitchenTicket.TicketStatus.NEW);
            ticket.setNotes(savedBatch.getNotes());
        ...
        // Hardware Print Routing: dispatch newly created batch tickets with exact unsent quantities
        try {
            printRoutingService.routeAndPrintKitchenBatches(createdBatches);
        } catch (Exception pex) {
            log.warn("Printer dispatch warning: {}", pex.getMessage());
        }

        // Targeted WebSocket notifications per kitchen station:
        for (KitchenOrderBatch b : createdBatches) {
            KitchenBatchDto.Response batchPayload = toBatchResponse(b);
            wsNotification.notifyKitchenBatchCreated(b.getKitchen().getId(), batchPayload);

            // Also send legacy order payload
            List<OrderItem> stationItems = b.getItems().stream()
                    .map(KitchenOrderBatchItem::getOrderItem)
                    .filter(java.util.Objects::nonNull)
                    .collect(Collectors.toList());
            OrderDto.Response legacyPayload = buildKitchenOrderPayload(order, b.getKitchen().getId(), stationItems);
            wsNotification.notifyKitchenNewTicket(b.getKitchen().getId(), legacyPayload);
        }
    ...
    @Transactional(readOnly = true)
    public List<KitchenBatchDto.Response> getOrderBatches(UUID orderId, UUID tenantId) {
        List<KitchenOrderBatch> batches = kitchenOrderBatchRepository.findByTenantIdAndOrderIdOrderByBatchNumberAsc(tenantId, orderId);
        return batches.stream().map(this::toBatchResponse).collect(Collectors.toList());
    }

    public KitchenBatchDto.Response toBatchResponse(KitchenOrderBatch b) {
        Order o = b.getOrder();
        List<KitchenBatchDto.ItemResponse> itemResponses = b.getItems() != null
                ? b.getItems().stream().map(this::toBatchItemResponse).collect(Collectors.toList())
                : Collections.emptyList();

        return KitchenBatchDto.Response.builder()
                .id(b.getId())
                .orderId(o.getId())
    ...
    public KitchenBatchDto.ItemResponse toBatchItemResponse(KitchenOrderBatchItem bi) {
        return KitchenBatchDto.ItemResponse.builder()
                .id(bi.getId())
