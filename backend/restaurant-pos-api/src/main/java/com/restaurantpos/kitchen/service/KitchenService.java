...
public class KitchenService {
    ...
    private final com.restaurantpos.kitchen.repository.KitchenOrderBatchRepository kitchenOrderBatchRepository;
    ...
    public void updateKitchenOrderStatusForKitchens(UUID orderId, UUID tenantId, String statusStr, Set<UUID> kitchenIds) {
        ...
        wsNotification.notifyOrderStatusChanged(tenantId, orderService.toResponse(saved));
        if (saved.getTable() != null) {
            wsNotification.notifyTableUpdated(tenantId, orderService.toTableResponse(saved.getTable(), saved));
        }
        if (kitchenIds != null) {
            for (UUID kId : kitchenIds) {
                wsNotification.notifyKitchenItemStatus(kId, Map.of(
                        "orderId", orderId,
                        "status", status.name()
                ));
            }
    ...
    @Transactional(readOnly = true)
    public List<com.restaurantpos.kitchen.dto.KitchenBatchDto.Response> getActiveKitchenBatches(UUID tenantId, UUID kitchenId) {
        return getActiveKitchenBatches(tenantId, kitchenId, true);
    }

    @Transactional(readOnly = true)
    public List<com.restaurantpos.kitchen.dto.KitchenBatchDto.Response> getActiveKitchenBatches(UUID tenantId, UUID kitchenId, boolean includeServed) {
        List<com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus> statuses = new java.util.ArrayList<>(List.of(
        ...
        if (includeServed) {
            statuses.add(com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus.SERVED);
            statuses.add(com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus.DELIVERED);
        }

        List<com.restaurantpos.kitchen.entity.KitchenOrderBatch> batches;
        if (kitchenId != null) {
            batches = kitchenOrderBatchRepository.findByTenantIdAndKitchenIdAndStatusInOrderByCreatedAtAsc(tenantId, kitchenId, statuses);
        } else {
            batches = kitchenOrderBatchRepository.findByTenantIdAndStatusInOrderByCreatedAtAsc(tenantId, statuses);
        }

        return batches.stream()
                .filter(b -> b.getOrder() != null && b.getOrder().getDeletedAt() == null && b.getOrder().getStatus() != Order.OrderStatus.CANCELLED)
                .map(this::toBatchResponse)
                .collect(Collectors.toList());
    }

    @Transactional(readOnly = true)
    public List<com.restaurantpos.kitchen.dto.KitchenBatchDto.Response> getActiveKitchenBatchesForKitchens(UUID tenantId, Set<UUID> kitchenIds) {
        return getActiveKitchenBatchesForKitchens(tenantId, kitchenIds, true);
    }

    @Transactional(readOnly = true)
    public List<com.restaurantpos.kitchen.dto.KitchenBatchDto.Response> getActiveKitchenBatchesForKitchens(UUID tenantId, Set<UUID> kitchenIds, boolean includeServed) {
        if (kitchenIds == null || kitchenIds.isEmpty()) {
            return Collections.emptyList();
        }
        List<com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus> statuses = new java.util.ArrayList<>(List.of(
        ...
        List<com.restaurantpos.kitchen.entity.KitchenOrderBatch> batches =
                kitchenOrderBatchRepository.findByTenantIdAndKitchenIdInAndStatusInOrderByCreatedAtAsc(tenantId, kitchenIds, statuses);
    ...
    @Transactional
    public com.restaurantpos.kitchen.dto.KitchenBatchDto.Response updateBatchStatus(UUID batchId, UUID tenantId, String statusStr, Set<UUID> allowedKitchenIds) {
        com.restaurantpos.kitchen.entity.KitchenOrderBatch batch = kitchenOrderBatchRepository.findByIdAndTenantId(batchId, tenantId)
                .orElseThrow(() -> PosException.notFound("Kitchen batch topilmadi: " + batchId));

        if (allowedKitchenIds != null && !allowedKitchenIds.isEmpty() && batch.getKitchen() != null) {
            if (!allowedKitchenIds.contains(batch.getKitchen().getId())) {
                throw PosException.forbidden("Sizda boshqa oshxona partiyasini o'zgartirish huquqi yo'q!");
            }
        }

        com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus newStatus =
                com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus.valueOf(statusStr.toUpperCase());
        batch.setStatus(newStatus);
        Instant now = Instant.now();

        if (newStatus == com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus.READY) {
            batch.setReadyAt(now);
        } else if (newStatus == com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus.SERVED || newStatus == com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus.DELIVERED) {
            batch.setServedAt(now);
        } else if (newStatus == com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus.CANCELLED) {
            batch.setCancelledAt(now);
        }

        // Propagate status to batch items
        if (batch.getItems() != null) {
            for (com.restaurantpos.kitchen.entity.KitchenOrderBatchItem item : batch.getItems()) {
                if (item.getStatus() != com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.CANCELLED) {
                    com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus itemStatus =
                            com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.valueOf(newStatus.name());
                    item.setStatus(itemStatus);
                    if (newStatus == com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus.READY) {
                        item.setReadyAt(now);
                    } else if (newStatus == com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus.SERVED) {
    ...
    public com.restaurantpos.kitchen.dto.KitchenBatchDto.ItemResponse updateBatchItemStatus(UUID batchItemId, UUID tenantId, String statusStr, Set<UUID> allowedKitchenIds) {
        ...
        com.restaurantpos.kitchen.entity.KitchenOrderBatch batch = item.getBatch();
        if (allowedKitchenIds != null && !allowedKitchenIds.isEmpty() && batch.getKitchen() != null) {
            if (!allowedKitchenIds.contains(batch.getKitchen().getId())) {
                throw PosException.forbidden("Sizda boshqa oshxona partiyasini o'zgartirish huquqi yo'q!");
            }
        ...
        com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus newStatus =
                com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.valueOf(statusStr.toUpperCase());
        item.setStatus(newStatus);
        Instant now = Instant.now();

        if (newStatus == com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.READY) {
            item.setReadyAt(now);
        } else if (newStatus == com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.SERVED || newStatus == com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.DELIVERED) {
            item.setServedAt(now);
        }

        com.restaurantpos.kitchen.entity.KitchenOrderBatchItem savedItem = kitchenOrderBatchItemRepository.save(item);

        if (batch.getItems() != null && !batch.getItems().isEmpty()) {
            boolean allReady = batch.getItems().stream()
                    .allMatch(i -> i.getStatus() == com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.READY
                            || i.getStatus() == com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.SERVED
                            || i.getStatus() == com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.CANCELLED);
            boolean allServed = batch.getItems().stream()
                    .allMatch(i -> i.getStatus() == com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.SERVED
                            || i.getStatus() == com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.CANCELLED);
            boolean anyCooking = batch.getItems().stream()
                    .anyMatch(i -> i.getStatus() == com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.COOKING
                            || i.getStatus() == com.restaurantpos.kitchen.entity.KitchenOrderBatchItem.ItemStatus.PREPARING);
            ...
            if (allServed) {
                ...
                batch.setReadyAt(now);
            } else if (anyCooking && batch.getStatus() == com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus.NEW) {
                batch.setStatus(com.restaurantpos.kitchen.entity.KitchenOrderBatch.BatchStatus.COOKING);
            }
            kitchenOrderBatchRepository.save(batch);
        }

        if (batch.getKitchen() != null) {
            wsNotification.notifyKitchenBatchUpdated(batch.getKitchen().getId(), toBatchResponse(batch));
        }

        return toBatchItemResponse(savedItem);
    }

    public com.restaurantpos.kitchen.dto.KitchenBatchDto.Response toBatchResponse(com.restaurantpos.kitchen.entity.KitchenOrderBatch b) {
        Order o = b.getOrder();
        List<com.restaurantpos.kitchen.dto.KitchenBatchDto.ItemResponse> itemResponses = b.getItems() != null
                ? b.getItems().stream().map(this::toBatchItemResponse).collect(Collectors.toList())
                : Collections.emptyList();

        return com.restaurantpos.kitchen.dto.KitchenBatchDto.Response.builder()
                .id(b.getId())
                .orderId(o.getId())
                ...
                .batchType(b.getBatchType().name())
                .status(b.getStatus().name())
                .notes(b.getNotes())
                .createdAt(b.getCreatedAt())
                .sentAt(b.getSentAt())
                .readyAt(b.getReadyAt())
                .servedAt(b.getServedAt())
                .cancelledAt(b.getCancelledAt())
                .items(itemResponses)
                .build();
    }

    public com.restaurantpos.kitchen.dto.KitchenBatchDto.ItemResponse toBatchItemResponse(com.restaurantpos.kitchen.entity.KitchenOrderBatchItem bi) {
        return com.restaurantpos.kitchen.dto.KitchenBatchDto.ItemResponse.builder()
                .id(bi.getId())
                .batchId(bi.getBatch() != null ? bi.getBatch().getId() : null)
                .orderItemId(bi.getOrderItem() != null ? bi.getOrderItem().getId() : null)
                .productId(bi.getProduct() != null ? bi.getProduct().getId() : null)
                .productName(bi.getProductName())
                .productSku(bi.getProductSku())
                .quantity(bi.getQuantity())
                .unitPrice(bi.getUnitPrice())
                .status(bi.getStatus().name())
                .notes(bi.getNotes())
                .readyAt(bi.getReadyAt())
                .totalOrderQuantity(bi.getOrderItem() != null ? bi.getOrderItem().getQuantity() : bi.getQuantity())
                .build();
    }
}
