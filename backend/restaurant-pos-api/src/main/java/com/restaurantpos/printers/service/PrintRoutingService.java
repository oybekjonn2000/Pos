...
public class PrintRoutingService {
    ...
    /**
     * Dispatches kitchen batches (initial or addons) to their respective kitchen primary printers.
     * CRITICAL: Printer errors or unassigned printers MUST NEVER abort or roll back the order.
     */
    @Async
    @Transactional
    public void routeAndPrintKitchenBatches(List<KitchenOrderBatch> batches) {
        if (batches == null || batches.isEmpty()) return;

        for (KitchenOrderBatch batch : batches) {
            Kitchen kitchen = batch.getKitchen();
            if (kitchen == null) continue;

            if (!kitchen.isAutoPrint()) {
                log.info("Kitchen '{}' auto_print is OFF. Skipping batch ticket print.", kitchen.getName());
                continue;
            }

            UUID tenantId = batch.getTenant().getId();
            ...
            printKitchenBatchSafely(printer, batch, false);
        }
    }

    private void printKitchenBatchSafely(Printer targetPrinter, KitchenOrderBatch batch, boolean isReprint) {
        try {
            kitchenPrintService.printKitchenBatchTicket(targetPrinter, batch, isReprint);
            ...
            log.info("Kitchen batch #{} ticket successfully printed to '{}' for '{}'",
                    batch.getBatchNumber(), targetPrinter.getName(), batch.getKitchen() != null ? batch.getKitchen().getName() : "");
        } catch (Exception ex) {
            ...
            if (targetPrinter.getFallbackPrinter() != null && targetPrinter.getFallbackPrinter().isActive()) {
                ...
                try {
                    kitchenPrintService.printKitchenBatchTicket(fallback, batch, isReprint);
                    ...
                    log.info("Fallback batch print SUCCESS: '{}'", fallback.getName());
                } catch (Exception fex) {
