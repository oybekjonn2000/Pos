...
public class WebSocketNotificationService {
    ...
    /** Aniq bitta oshxonaga yangi partiya keldi (/topic/kitchen/{kitchenId}) */
    public void notifyKitchenBatchCreated(UUID kitchenId, Object batchPayload) {
        String destination = "/topic/kitchen/" + kitchenId;
        messagingTemplate.convertAndSend(destination, new WebSocketEvent("KITCHEN_ORDER_BATCH_CREATED", batchPayload));
        log.debug("WS: batch created for kitchen {} -> {}", kitchenId, destination);
    }

    /** Aniq bitta oshxonada partiya holati o'zgardi (/topic/kitchen/{kitchenId}) */
    public void notifyKitchenBatchUpdated(UUID kitchenId, Object batchPayload) {
        String destination = "/topic/kitchen/" + kitchenId;
        messagingTemplate.convertAndSend(destination, new WebSocketEvent("KITCHEN_ORDER_BATCH_UPDATED", batchPayload));
        log.debug("WS: batch updated for kitchen {} -> {}", kitchenId, destination);
    }
