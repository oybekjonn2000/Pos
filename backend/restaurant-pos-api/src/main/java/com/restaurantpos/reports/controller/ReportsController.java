...
public class ReportsController {
    ...
    private Instant[] parseDateRange(LocalDate dateFrom, LocalDate dateTo) {
        ...
        Instant to = toDate.plusDays(1).atStartOfDay(TASHKENT_ZONE).minusNanos(1_000_000).toInstant();
    ...
    public ResponseEntity<ApiResponse<ReportDto.SalesSummary>> getSalesSummary(
        ...
        return ResponseEntity.ok(ApiResponse.success(
                reportsService.getSalesSummary(user.getTenantId(), range[0], range[1], waiterId, kitchenId, user)));
    }
    ...
    public ResponseEntity<ApiResponse<List<ReportDto.ProductSaleItem>>> getProductSales(
        ...
        Instant[] range = parseDateRange(dateFrom, dateTo);
        return ResponseEntity.ok(ApiResponse.success(
                reportsService.getProductSalesReport(user.getTenantId(), range[0], range[1], categoryId, kitchenId, waiterId, user)));
    }
    ...
    public ResponseEntity<ApiResponse<ReportDto.ProfitLoss>> getProfitLoss(
        ...
        Instant[] range = parseDateRange(dateFrom, dateTo);
        return ResponseEntity.ok(ApiResponse.success(
                reportsService.getProfitLossReport(user.getTenantId(), range[0], range[1], waiterId, kitchenId, user)));
    }
    ...
    public ResponseEntity<ApiResponse<List<ReportDto.CashierSummary>>> getCashierReport(
        ...
        Instant[] range = parseDateRange(dateFrom, dateTo);
        return ResponseEntity.ok(ApiResponse.success(
                reportsService.getCashierReport(user.getTenantId(), range[0], range[1], waiterId, kitchenId, user)));
    }
    ...
    public ResponseEntity<ApiResponse<List<ReportDto.WaiterPerformance>>> getWaiterReport(
        ...
        Instant[] range = parseDateRange(dateFrom, dateTo);
        return ResponseEntity.ok(ApiResponse.success(
                reportsService.getWaiterReport(user.getTenantId(), range[0], range[1], waiterId, kitchenId, user)));
    }
    ...
    public ResponseEntity<ApiResponse<List<ReportDto.KitchenPerformance>>> getKitchenReport(
        ...
        Instant[] range = parseDateRange(dateFrom, dateTo);
        return ResponseEntity.ok(ApiResponse.success(
                reportsService.getKitchenReport(user.getTenantId(), range[0], range[1], kitchenId, waiterId, user)));
    }
